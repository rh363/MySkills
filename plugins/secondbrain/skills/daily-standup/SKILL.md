...
- Every line is a `- [ ]` checkbox. When the task is completed during the day, the checkbox is flipped **in place** to `- [x]` (plain text — **no strikethrough**, `~~...~~` breaks Obsidian rendering). The line **stays in `## Schedule`** so the plan you made in the morning remains visible as memory. The same completed task is **also copied** (not moved) into `## Done today`, which acts as the flat consolidated list of everything completed today (Schedule + ad-hoc).
